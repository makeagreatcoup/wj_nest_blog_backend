...
import { OrderType, SelectTrashMode } from '../core/constants';
 ...
 * 分页选项
 */
export interface IPaginateOptions<C extends IPaginationMeta = IPaginationMeta>
  extends IPaginationOptions<C> {
  /**
   * 当前页数
   */
  page: number;
 ...
 * 分页返回数据
 */
export interface IPaginateResult<T extends ObjectLiteral> {
  /**
   * 分页数据
   */
  items: T[];
...
export type ServiceListQueryOptionWithTrashed<E extends ObjectLiteral> = Omit<
  ...
  'withTrashed'
> & { trashed?: `${SelectTrashMode}` } & Record<string, any>;

/**
 * 不带软删除的服务类数据列表查询类型
 */
export type ServiceListQueryOptionNotWithTrashed<E extends ObjectLiteral> =
