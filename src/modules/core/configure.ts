...
export class Configure {
  ...
  set<T>(key: string, value: T, storage = false, append = false) {
    if (storage && this.storage) {
      fs.ensureFileSync(this.yamlPath);
      set(this.ymlConfig, key, value);
      fs.writeFileSync(this.yamlPath, JSON.stringify(this.ymlConfig, null, 4));
  ...
  protected enabledStorage() {
    ...
    fs.ensureFileSync(this.yamlPath);
    const ymlConfig = YAML.parse(fs.readFileSync(this.yamlPath, 'utf8'));
